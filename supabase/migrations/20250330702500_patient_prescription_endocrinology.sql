/*
  # Patient Prescriptions: Endocrinology Providers with Synthetic Patients
  
  This migration populates the patient_prescriptions table with data for
  Endocrinology providers. It uses synthetic patient IDs instead of
...
DECLARE
  -- Batch identifier
  batch_id CONSTANT INTEGER := 5;
  specialty_name CONSTANT TEXT := 'Endocrinology';
...
BEGIN
  ...
  SELECT 
    ...
    CASE
      -- Endocrinologists tend to prescribe more brand-name medications
      WHEN practice_size IN ('hospital', 'academic') THEN 0.75  -- Higher brand name preference in academic/hospital
      WHEN practice_size = 'large' THEN 0.65
      WHEN practice_size = 'solo' THEN 0.55
      ELSE 0.6
  ...
  -- Find appropriate medications for endocrinology providers
  ...
  FROM medications
  WHERE 
    -- Primarily endocrine and diabetes medications
    category = 'Endocrine' OR
    category = 'Diabetes' OR 
    -- Specific endocrine-related medications
    name ILIKE '%thyroid%' OR
    name ILIKE '%insulin%' OR
    name ILIKE '%hormone%' OR
    name ILIKE '%adrenal%' OR
    name ILIKE '%pituitary%' OR
    name ILIKE '%parathyroid%';
  ...
  RAISE NOTICE 'Completed batch % with % Endocrinology providers and % synthetic patients each', 
