...
BEGIN
  ...
  FOR i IN 0..(target_count / batch_size) LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      -- If there's an error, rollback this batch and continue
...
BEGIN
  ...
  LOOP
    BEGIN -- Begin transaction for this provider
      ...
      RAISE NOTICE 'Processing provider % of %: %', provider_count, total_provider_count, provider_rec.provider_id;
      ...
      -- Generate prescriptions in batches
      batch_counter := 0;
      ...
      FOR i IN 1..prescription_count LOOP
        ...
        FOR j IN 1..prescriptions_per_patient LOOP
          ...
          BEGIN
            ...
            -- If we've reached the batch size, commit this batch
    ...
    IF provider_count % 10 = 0 THEN
      RAISE NOTICE 'Processed % of % providers. Generated % prescriptions so far.', 
        provider_count, total_provider_count, total_prescriptions;
    END IF;
  END LOOP;
