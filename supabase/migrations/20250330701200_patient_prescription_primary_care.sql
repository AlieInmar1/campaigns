/*
  # Patient Prescriptions: Primary Care Providers with Synthetic Patients
  
  This migration populates the patient_prescriptions table with data for
  Primary Care providers. It uses synthetic patient IDs instead of
...
DECLARE
  -- Batch identifier
  batch_id CONSTANT INTEGER := 3;
  specialty_name CONSTANT TEXT := 'Primary Care';
...
BEGIN
  ...
  SELECT 
    ...
    CASE
      WHEN practice_size IN ('hospital', 'academic') THEN 0.65  -- More likely to prescribe brand names
      WHEN practice_size = 'large' THEN 0.55
  ...
  -- Find appropriate medications for primary care providers
  ...
  RAISE NOTICE 'Completed batch % with % Primary Care providers and % synthetic patients each', 
