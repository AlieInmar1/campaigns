/*
  # Add 10k Prescriptions Per Provider
  
  This migration generates 10,000 prescriptions for each provider with:
...
-- First, ensure we have a patients table to track patient data
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'patients') THEN
    ...
    CREATE INDEX idx_patients_geographic_area ON patients(geographic_area);
...
-- Ensure we have a prescriptions table with the right structure
DO $$
BEGIN
...
BEGIN
  ...
  RAISE NOTICE 'Prescription generation complete. Total prescriptions: %', (SELECT COUNT(*) FROM prescriptions);
END $$;
