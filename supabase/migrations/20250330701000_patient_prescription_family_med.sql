/*
  # Patient Prescriptions: Family Medicine Providers with Synthetic Patients
  
  This migration drops and recreates the patient_prescriptions table, then populates
  it with data for Family Medicine providers. It uses synthetic patient IDs instead of
...
DROP TABLE IF EXISTS public.patient_prescriptions CASCADE;

-- Create a new version of the table with the batch_id included in the structure
...
CREATE INDEX idx_patient_prescriptions_provider_id ON public.patient_prescriptions(provider_id);
CREATE INDEX idx_patient_prescriptions_patient_id ON public.patient_prescriptions(patient_id);
CREATE INDEX idx_patient_prescriptions_medication_id ON public.patient_prescriptions(medication_id);
CREATE INDEX idx_patient_prescriptions_prescription_date ON public.patient_prescriptions(prescription_date);
CREATE INDEX idx_patient_prescriptions_batch_id ON public.patient_prescriptions(batch_id);

-- Add comments to the table and columns for better documentation
COMMENT ON TABLE public.patient_prescriptions IS 'Prescriptions written by providers to patients';
COMMENT ON COLUMN public.patient_prescriptions.batch_id IS 'Identifies which batch migration created this record';

-- Main procedure to generate prescriptions
DO $$
DECLARE
  -- Batch identifier
  batch_id CONSTANT INTEGER := 1;
  specialty_name CONSTANT TEXT := 'Family Medicine';
...
BEGIN
  ...
  SELECT 
    ...
    CASE
      ...
      WHEN practice_size = 'solo' THEN 0.4  -- Less likely to prescribe brand names
      ELSE 0.5
  ...
  -- Find appropriate medications for family medicine providers
  ...
  RAISE NOTICE 'Completed batch % with % Family Medicine providers and % synthetic patients each', 
